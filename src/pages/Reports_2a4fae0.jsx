...
export default function Reports() {
  ...
  return (
    <div className="w-full max-w-sm mx-auto md:max-w-2xl lg:max-w-4xl xl:max-w-6xl px-4 sm:px-8 space-y-6 pb-12 pt-2 sm:pt-4 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Business Reports</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 font-medium italic">"Data is the new oil of the digital economy."</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
         ...
         <div className="md:col-span-2 flex bg-slate-100 dark:bg-slate-700/50 p-1 rounded-2xl border border-slate-200 dark:border-slate-600">
            {[
              ...
              <button
                ...
                className={`flex-1 py-1.5 px-3 text-[10px] sm:text-xs font-black rounded-xl transition-all uppercase tracking-widest ${timeFilter === f.id ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}
              >
                {f.label}
              </button>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-600 relative overflow-hidden group hover:shadow-md transition-shadow">
           <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-50 dark:bg-indigo-900/20 rounded-bl-full -mr-10 -mt-10 group-hover:scale-125 transition-transform duration-500"></div>
           ...
           <h3 className="text-3xl font-black text-slate-800 dark:text-slate-100">₹{stats.totalSales.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h3>
           <p className="text-slate-400 text-xs mt-4 font-bold tracking-tight">{stats.totalBills} Bills generated</p>
        </div>

        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-600 relative overflow-hidden group hover:shadow-md transition-shadow">
           <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-50 dark:bg-emerald-900/20 rounded-bl-full -mr-10 -mt-10 group-hover:scale-125 transition-transform duration-500"></div>
           <p className="text-emerald-500 dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
             <TrendingUp size={12} /> GP (Gross Profit)
           </p>
           <h3 className="text-3xl font-black text-emerald-600 dark:text-emerald-400">₹{stats.totalProfit.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h3>
           <p className="text-emerald-600/60 dark:text-emerald-400/60 text-xs mt-4 font-bold flex items-center gap-1">
             <TrendingUp size={12} /> Profit potential
           </p>
        </div>

        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-600 relative overflow-hidden group hover:shadow-md transition-shadow">
           <div className="absolute top-0 right-0 w-24 h-24 bg-blue-50 dark:bg-blue-900/20 rounded-bl-full -mr-10 -mt-10 group-hover:scale-125 transition-transform duration-500"></div>
           <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
             <AlertCircle size={12} className="text-blue-500" /> Total Collected
           </p>
           <h3 className="text-3xl font-black text-slate-800 dark:text-slate-100">₹{stats.totalPaid.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h3>
           <p className="text-slate-400 text-xs mt-4 font-bold">Successfully received</p>
        </div>

        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-600 relative overflow-hidden group hover:shadow-md transition-shadow">
           <div className="absolute top-0 right-0 w-24 h-24 bg-red-50 dark:bg-red-900/20 rounded-bl-full -mr-10 -mt-10 group-hover:scale-125 transition-transform duration-500"></div>
           <p className="text-red-500 dark:text-red-400 text-[10px] font-black uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
             <AlertCircle size={12} /> Market Credit
           </p>
           <h3 className={`text-3xl font-black ${stats.totalOutstanding > 0 ? 'text-red-500 dark:text-red-400' : 'text-slate-800 dark:text-slate-100'}`}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-600 overflow-hidden">
        <div className="p-6 border-b border-slate-50 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center px-8">
        ...
        <div className="overflow-x-auto overflow-y-auto max-h-[500px] custom-scrollbar">
           {filteredBills.length === 0 ? (
             ...
             <table className="w-full text-left border-collapse min-w-full max-w-sm">
               <thead className="sticky top-0 bg-white dark:bg-slate-800/90 backdrop-blur-md z-10 border-b border-slate-100 dark:border-slate-700 shadow-sm">
               ...
               <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                 {filteredBills.map((bill, idx) => (
                   <tr key={idx} className="hover:bg-slate-50/80 dark:hover:bg-slate-700/50 transition-all duration-300 group">
