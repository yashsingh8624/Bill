  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-12 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Business Reports</h2>
          <p className="text-slate-500 text-sm mt-1 font-medium italic">Detailed financial insights for your shop.</p>
        </div>
        <div className="flex flex-wrap gap-2">
           <button 
             onClick={() => setShowExcelView(!showExcelView)}
             disabled={filteredBills.length === 0}
             className="bg-white border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 text-indigo-700 px-4 py-2.5 rounded-xl transition-all font-bold flex items-center gap-2 shadow-sm text-sm"
           >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
         <div className="md:col-span-2 bg-white p-2.5 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3 px-4">
         ...
         <div className="md:col-span-2 flex bg-slate-100 p-1 rounded-2xl border border-slate-200">
            {[
              ...
              <button
                ...
                className={`flex-1 py-1.5 px-3 text-[10px] font-black rounded-xl transition-all uppercase tracking-widest ${timeFilter === f.id ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                {f.label}
              </button>
            ))}
      ...
      {timeFilter === 'CUSTOM' && (
        <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-wrap items-center gap-4 animate-in slide-in-from-top duration-300">
          <div className="flex items-center gap-2">
            ...
            <input 
              ...
              className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-sm font-bold focus:ring-2 focus:ring-indigo-500/50 outline-none"
            />
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-shadow">
           ...
           <h3 className="text-3xl font-black text-slate-800">₹{stats.totalSales.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h3>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-shadow">
           ...
           <h3 className={`text-3xl font-black ${stats.netProfit >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
             ₹{stats.netProfit.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
           </h3>
           <p className="text-slate-400 text-xs mt-4 font-bold flex flex-col gap-0.5">
             <span className="flex justify-between">GP Margin: <span>₹{stats.totalGP.toFixed(2)}</span></span>
             <span className="flex justify-between text-rose-400">Expenses: <span>- ₹{stats.totalExpenses.toFixed(2)}</span></span>
           </p>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-shadow">
           ...
           <h3 className="text-3xl font-black text-slate-800">₹{stats.totalPaid.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h3>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-shadow">
           ...
           <h3 className={`text-3xl font-black ${stats.totalOutstanding > 0 ? 'text-red-500' : 'text-slate-800'}`}>
         ...
         <div className="overflow-x-auto overflow-y-auto max-h-[500px] custom-scrollbar">
           {filteredBills.length === 0 ? (
             <div className="p-24 text-center text-slate-400">
               <div className="bg-slate-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 border border-slate-100 shadow-inner group transition-all">
                 <Search size={32} className="text-slate-300 group-hover:scale-110 transition-transform" />
               </div>
               <p className="font-black text-xl text-slate-700 mb-1">No sales data found</p>
             ...
             <table className="w-full text-left border-collapse whitespace-nowrap text-sm">
               ...
               <tbody className="divide-y divide-slate-100">
                 {filteredBills.map((b, idx) => (
                   <tr key={idx} className="hover:bg-slate-50 text-slate-700 font-medium tracking-tight">
                     ...
                     <td className="py-2 px-4 font-bold text-slate-900">₹{(Array.isArray(b.items) && b.items.length > 0 ? b.items.reduce((s, i) => s + (parseFloat(i.amount) || 0), 0) : ((parseFloat(b.subTotal) || 0) + (parseFloat(b.cgst) || 0) + (parseFloat(b.sgst) || 0))).toFixed(2)}</td>
                     <td className="py-2 px-4 text-emerald-600">₹{(b.amountPaid || 0).toFixed(2)}</td>
                     <td className="py-2 px-4 text-red-500">₹{(b.outstanding || 0).toFixed(2)}</td>
                   ...
                   <tr key={idx} className="hover:bg-slate-50/80 transition-all duration-300 group">
                      <td className="py-5 px-8">
                         <div className="flex items-center gap-4">
                            <div className="bg-slate-100 text-slate-500 p-2.5 rounded-2xl group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm">
                               <FileText size={18} />
                            </div>
                            <div>
                      ...
                      <td className="py-5 px-6">
                         <p className="font-bold text-slate-700 text-sm">{bill.customerName}</p>
                         <p className="text-[10px] text-slate-400 font-medium uppercase tracking-tighter">{bill.paymentMode} Payment</p>
                      </td>
                      <td className="py-5 px-6 text-right">
                         <p className="font-black text-slate-800 text-sm">₹{bill.amountPaid.toFixed(2)}</p>
                      ...
                      <td className="py-5 px-8 text-right">
                         <p className="font-black text-slate-900 text-lg tracking-tight">₹{(bill.grandTotal || bill.total || 0).toFixed(2)}</p>
