...
export default function Dashboard() {
  ...
  const chartData = analyticsData.map((d, i) => {
     let trend = 'flat';
     if (i > 0) {
       trend = d.sales > analyticsData[i-1].sales ? 'up' : d.sales < analyticsData[i-1].sales ? 'down' : 'flat';
     }
     return { label: d.name, value: d.sales, trend };
  });
return (
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative overflow-hidden group">
          ...
          <div className="flex flex-wrap items-center gap-4 relative z-10">
            {(!folderId || !spreadsheetId) && status !== 'ready' ? (
              ...
              <>
                {folderId && (
                  <a href={`https://drive.google.com/drive/folders/${folderId}`} target="_blank" rel="noopener noreferrer" className="bg-blue-50 border border-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white px-5 py-3 rounded-2xl font-bold flex items-center gap-2 transition-all shadow-sm hover:shadow-md hover:shadow-blue-500/20">
                    <FolderOpen size={18} />
                    Open Drive Folder
                  </a>
                )}
                {spreadsheetId && spreadsheetId !== 'LOCAL_MODE' && (
                  <a href={`https://docs.google.com/spreadsheets/d/${spreadsheetId}`} target="_blank" rel="noopener noreferrer" className="bg-emerald-50 border border-emerald-100 text-emerald-600 hover:bg-emerald-600 hover:text-white px-5 py-3 rounded-2xl font-bold flex items-center gap-2 transition-all shadow-sm hover:shadow-md hover:shadow-emerald-500/20">
