  ...
  return (
    <div className="space-y-6 w-full max-w-sm mx-auto md:max-w-2xl lg:max-w-4xl xl:max-w-6xl px-4 sm:px-8 pb-24 sm:pb-8 pt-2 sm:pt-4 min-w-0 page-animate">
      <div className="flex justify-between items-end flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Day Closing</h2>
        ...
        <div className="flex items-center gap-3">
          {isClosed && (
            <span className="bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 text-xs font-black px-3 py-1.5 rounded-xl flex items-center gap-1.5">
              <Lock size={12} /> Day Closed
            </span>
          )}
          <button
            onClick={() => setShowHistory(!showHistory)}
            className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-2 shadow-sm transition-colors"
          >
            <History size={14} /> View History
          </button>
          {!isClosed && (
            <div className="bg-white dark:bg-slate-800 px-4 py-2 rounded-xl shadow-sm border border-slate-100 dark:border-slate-600 flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
              <span className="text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest">Live</span>
            </div>
          )}
        </div>
      </div>
      ...
      {showHistory && (
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-600 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between bg-slate-50 dark:bg-slate-900/50">
            <h3 className="font-black text-slate-800 dark:text-slate-100 flex items-center gap-2"><History size={16} /> Closed Day Reports</h3>
            <button onClick={() => setShowHistory(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"><X size={18} /></button>
          </div>
            ...
            {pastReports.length === 0 ? (
              ...
              pastReports.map((r, i) => (
                <div key={i} className="px-6 py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <div>
                    <p className="font-black text-slate-800 dark:text-slate-100 text-sm">{new Date(r.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                  ...
                  <div className="text-right">
                    <p className="font-black text-slate-800 dark:text-slate-100">₹{parseFloat(r.totalSales || 0).toFixed(2)}</p>
                    <p className="text-xs text-emerald-600 dark:text-emerald-400 font-bold">Collected: ₹{parseFloat(r.totalCollected || 0).toFixed(2)}</p>
                  </div>
                </div>
              ))
            )}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-600 overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
          <h3 className="font-black text-slate-800 dark:text-slate-100 uppercase tracking-wider text-sm">Today's Transactions</h3>
          <span className="text-xs font-bold text-slate-500 dark:text-slate-400">{todayBills.length} Bills</span>
        </div>
        <div className="divide-y divide-slate-100 dark:divide-slate-700/50 max-h-[350px] overflow-y-auto custom-scrollbar">
          {todayBills.length === 0 ? (
              ...
              return (
                <div key={idx} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className={`p-2 rounded-xl ${bill.paymentMode === 'Credit' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' : 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400'}`}>
                      <Receipt size={18} />
                    </div>
                    <div>
                      <p className="font-bold text-slate-800 dark:text-slate-100 text-sm">#{bill.invoiceNo} — {bill.customerName}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{bill.paymentMode} Mode</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-black text-slate-800 dark:text-slate-100">₹{billTotal.toFixed(2)}</p>
                    <p className={`text-[10px] font-black uppercase ${parseFloat(bill.outstanding || 0) > 0 ? 'text-red-500 dark:text-red-400' : 'text-emerald-500 dark:text-emerald-400'}`}>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-6 bg-slate-800 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden">
        ...
        {!isClosed && (
          <button
            ...
            className="group relative overflow-hidden px-10 py-4 rounded-2xl font-black uppercase tracking-widest text-sm transition-all shadow-xl bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 hover:scale-105 active:scale-95 border border-transparent dark:border-slate-600"
          >
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-slate-900/60 z-[200] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 border border-slate-100 dark:border-slate-600">
            <div className="px-6 py-5 border-b border-amber-100 dark:border-amber-900 bg-amber-50 dark:bg-amber-900/20 flex items-center justify-between">
            ...
            <div className="p-6 space-y-4">
              <p className="text-slate-600 dark:text-slate-400 font-medium text-sm leading-relaxed">
                You are about to close <span className="font-black text-slate-800 dark:text-slate-100">{new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}</span>.
                This will save the following summary to your daily reports:
              </p>
              <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-100 dark:border-slate-700 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400 font-medium">Total Bills</span><span className="font-black text-slate-800 dark:text-slate-100">{stats.billCount}</span></div>
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400 font-medium">Total Sales</span><span className="font-black text-slate-800 dark:text-slate-100">₹{stats.totalSales.toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400 font-medium">Total Collected</span><span className="font-black text-emerald-600 dark:text-emerald-400">₹{stats.totalCollected.toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400 font-medium">Udhaar (Outstanding)</span><span className="font-black text-amber-600 dark:text-amber-500">₹{stats.totalOutstanding.toFixed(2)}</span></div>
              </div>
            ...
            <div className="px-6 pb-6 flex gap-3">
              <button
                ...
                className="flex-1 py-3 border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold rounded-xl transition-colors"
              >
                Cancel
              </button>
              <button
