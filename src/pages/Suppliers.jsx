    ...
    <div className="space-y-6 flex flex-col h-[calc(100vh-8rem)] relative w-full page-animate">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-800 tracking-tight">Suppliers</h2>
            <p className="text-slate-500 text-sm font-bold">Manage purchases, payments, and supplier balances.</p>
          </div>
        </div>
        <button 
          onClick={() => navigate('/add-party?role=supplier')}
          className="flex items-center gap-2 bg-amber-600 hover:bg-amber-500 text-white px-5 py-3 rounded-2xl font-black shadow-lg shadow-amber-200 transition-all active:scale-95 whitespace-nowrap"
        >
          <Plus size={20} strokeWidth={3} /> Add Supplier
        </button>
      </div>

      <div className="search-wrapper bg-white p-2.5 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3 px-5 flex-shrink-0 transition-all focus-within:ring-2 focus-within:ring-amber-500/20 focus-within:border-amber-300">
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead className="sticky top-0 bg-slate-50/95 backdrop-blur-sm z-10 border-b border-slate-200">
                <tr className="text-slate-500 text-[10px] uppercase font-black tracking-widest">
                  <th className="py-5 px-8">Supplier Info</th>
                  <th className="py-5 px-8 text-right">Payable Amount</th>
                ...
                {filteredSuppliers.map(supplier => (
                  ...
                  >
                     <td className="py-5 px-8 flex items-center gap-5">
                        <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg uppercase shadow-sm bg-amber-50 text-amber-600 border border-amber-100 group-hover:scale-110 transition-transform">
                           {(supplier.displayName || '??').substring(0,2)}
                        </div>
                        <div>
                           <p className="font-black text-slate-800 text-[16px]">{supplier.displayName}</p>
                     ...
                     <td className="py-5 px-8 text-right">
                        <div className="flex flex-col items-end gap-1.5">
                           <span className="font-black text-[17px] text-slate-800">{supplier.amountText}</span>
                           {supplier.balance > 0 ? (
                             <span className="text-[10px] uppercase font-black tracking-widest px-2.5 py-1 rounded-lg shadow-sm bg-rose-100 text-rose-700 border border-rose-200/50">
                               You'll Give
                             </span>
                           ) : (
                     ...
                     <td className="py-5 px-8 text-center">
                        <div className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-slate-50 text-slate-300 group-hover:bg-amber-600 group-hover:text-white transition-all transform group-hover:translate-x-1">
