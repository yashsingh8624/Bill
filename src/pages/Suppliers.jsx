  ...
  if (loading) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-slate-500 dark:text-slate-500 transition-colors duration-300 font-bold animate-pulse">Loading Suppliers...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 w-full max-w-sm mx-auto md:max-w-2xl lg:max-w-4xl xl:max-w-6xl px-4 sm:px-8 pb-24 sm:pb-8 pt-2 sm:pt-4 min-w-0 page-animate flex flex-col" style={{ minHeight: 'calc(100vh - 10rem)' }}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="bg-amber-500 text-white p-2.5 rounded-2xl shadow-lg shadow-amber-200 dark:shadow-amber-900/20">
            <Truck className="w-5 h-5 sm:w-6 sm:h-6" />
          </div>
          <div>
            <h2 className="text-xl sm:text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight transition-colors duration-300">Suppliers</h2>
            <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm font-bold transition-colors duration-300">Manage purchases, payments, and supplier balances.</p>
          </div>
        </div>
        <button 
          onClick={() => navigate('/add-party?role=supplier')}
          className="flex items-center justify-center w-full sm:w-auto gap-2 bg-green-600 hover:bg-green-700 text-white px-5 py-3 rounded-2xl font-black shadow-lg shadow-green-600/30 transition-all duration-300 active:scale-95 whitespace-nowrap"
        >
              ...
              <tbody className="divide-y divide-slate-100/80 dark:divide-slate-800/80">
                {filteredSuppliers.map(supplier => (
                  <tr 
                    ...
                    className="hover:bg-amber-50/30 dark:hover:bg-amber-900/10 transition-all cursor-pointer group"
                  >
                     <td className="py-4 px-4 sm:py-5 sm:px-8 flex items-center gap-3 sm:gap-5">
                        <div className="w-10 h-10 sm:w-12 sm:h-12 flex-shrink-0 rounded-2xl flex items-center justify-center font-black text-sm sm:text-lg uppercase shadow-sm bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 border border-amber-100 dark:border-amber-800/50 group-hover:scale-110 transition-transform">
                     ...
                     <td className="py-4 px-3 sm:py-5 sm:px-8 text-center">
                        <div className="inline-flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-slate-50 dark:bg-slate-900/50 transition-colors duration-300 dark:bg-slate-800 text-slate-300 dark:text-slate-600 dark:text-slate-400 transition-colors duration-300 group-hover:bg-amber-600 group-hover:text-white transition-all transform group-hover:translate-x-1">
