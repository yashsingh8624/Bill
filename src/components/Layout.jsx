...
import { LayoutDashboard, Package, FileText, History, Users, Truck, Menu, Settings, X, FileSpreadsheet, Search, Sun, Plus, Activity, Banknote, LogOut } from 'lucide-react';
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-white/80 backdrop-blur-xl border-r border-slate-200 shadow-sm z-20 h-screen sticky top-0">
        <div className="h-20 flex items-center px-6 border-b border-slate-100 gap-4">
          ...
          <div className="overflow-hidden">
            <h1 className="text-lg font-black text-slate-800 leading-tight truncate">{userSettings?.businessName || 'Akash Traders'}</h1>
          ...
          {user && (
            <div className="flex items-center gap-3 mb-1 px-2">
              {user.picture ? (
                ...
                <div className="w-8 h-8 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-xs">{(user.name || 'U')[0]}</div>
              )}
      ...
      <div className="flex-1 flex flex-col relative bg-[#f5f7fb] min-w-0 w-full">
        ...
        <header className="lg:hidden h-16 flex items-center justify-between px-4 bg-white/80 backdrop-blur-xl border-b border-slate-200 shadow-sm z-30 sticky top-0">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-black text-slate-800 truncate max-w-[150px]">{userSettings?.businessName || 'Akash Traders'}</h1>
          </div>
          <button 
        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-200 flex items-center justify-around z-40 px-2 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
           ...
           <Link to="/customers" className={`flex flex-col items-center gap-1 transition-all ${location.pathname === '/customers' ? 'text-indigo-600 bg-indigo-50/50 rounded-xl px-3 py-1 scale-110' : 'text-slate-400'}`}>
               <Users size={20} />
