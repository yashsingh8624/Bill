...
import { Users, Search, IndianRupee, MessageCircle, ChevronRight, X, Receipt, Edit2, Trash2, ExternalLink } from 'lucide-react';
...
export default function CustomerLedger({ overrideCustomer = null, onBack = null }) {
  ...
  const handleEditSubmit = (e) => {
    e.preventDefault();
    if (!selectedCustomer || !updateCustomer) return;
  ...
  const currentBalance = currentCustomer 
    ? parseFloat(calculateCustomerBalance(ledger, currentCustomer.id, currentCustomer) || 0) 
    : 0;
  ...
  const getCustomerTotals = (customer) => {
    ...
    const totalBilled = entries
      .filter(e => e && (String(e.type).toUpperCase() === 'SALE' || String(e.type).toUpperCase() === 'OPENING'))
      .reduce((s, e) => s + parseFloat(e.amount || 0), 0);
      
    const totalPaid = entries
      .filter(e => e && (String(e.type).toUpperCase() === 'PAYMENT_IN' || String(e.type).toUpperCase() === 'PAYMENT' || String(e.type).toUpperCase() === 'ROLLOVER'))
      .reduce((s, e) => s + parseFloat(e.amount || 0), 0);
    ...
    const totalBills = entries.filter(e => e && String(e.type).toUpperCase() === 'SALE').length;
  ...
  const handleSendWhatsApp = () => {
    ...
    const bizName = userSettings.businessName || userSettings.ownerName || 'Business';
    const message = `Hello ${currentCustomer.name || 'Customer'},\nYour outstanding amount is ₹${outstanding.toFixed(2)}. Please clear your dues. Thank you.\n- ${bizName}`;
  ...
  const handleDownloadLedger = async () => {
    ...
    try {
      ...
      if (navigator.share) {
        ...
        } catch (shareErr) { /* ignore cancelled share */ }
      }
        ...
        <>
          ...
          <div className="bg-white dark:bg-slate-800 p-2.5 rounded-xl shadow-sm border border-slate-100 dark:border-slate-600 flex items-center gap-3 px-4 flex-shrink-0">
            ...
            <input 
              ...
              className="bg-transparent text-slate-800 dark:text-slate-100 flex-1 py-1 focus:outline-none placeholder:text-slate-400 font-medium"
              value={searchTerm}
            ...
            <div className="overflow-x-auto overflow-y-auto flex-1 h-full custom-scrollbar">
              {customers.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center p-12 text-center text-slate-500">
                  ...
                  <p className="font-bold text-slate-700 dark:text-slate-300 text-xl">No customers yet</p>
                  <p className="text-sm mt-2 font-medium">Customers are automatically saved when you generate a bill.</p>
                </div>
              ) : (
                <table className="w-full text-left border-collapse min-w-full max-w-sm sm:min-w-full">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                    {filteredCustomers.map(customer => {
                       const outstandingBalance = parseFloat(calculateCustomerBalance(ledger, customer.id, customer) || 0);
                       return (
                        <tr 
                          key={customer.id} 
                        ...
                        >
                           <td className="py-4 px-6 text-slate-800 dark:text-slate-100 font-bold flex items-center gap-3">
                              <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 flex items-center justify-center font-bold text-xs uppercase">
                                 {String(customer.name || '??').substring(0,2)}
                              </div>
                              {customer.name || 'Unnamed Customer'}
</td>
                           <td className="py-4 px-6 text-slate-600 dark:text-slate-400 font-medium">{customer.phone || '-'}</td>
                             <td className="py-4 px-6 text-right">
                               <span className={`font-black ${outstandingBalance > 0 ? 'text-red-500 bg-red-50 px-3 py-1.5 border-red-100' : 'text-emerald-600 bg-emerald-50 px-3 py-1.5 border-emerald-100'} rounded-lg border`}>
                                 ₹{outstandingBalance.toFixed(2)}
                               </span>
                            </td>
                           <td className="py-4 px-6 text-center">
                              <div className="flex justify-center text-slate-400 group-hover:text-indigo-600 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 transition-all rounded-lg p-1.5 shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className="p-2.5 text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 shadow-sm rounded-xl transition-all border border-slate-200 dark:border-slate-600"
                >
                ...
                <div>
                  <h2 className="text-2xl font-black text-slate-800 dark:text-slate-100 flex items-center gap-3">
                    ...
                    <button onClick={() => openEditModal(currentCustomer)} className="text-slate-400 hover:text-indigo-600" title="Edit">
                    <Edit2 size={16} />
                    </button>
                    <button onClick={handleDelete} className="text-slate-400 hover:text-red-600" title="Delete">
                       <Trash2 size={16} />
                    </button>
                  </h2>
              ...
              <div className="flex flex-wrap gap-2">
                <button 
                  ...
                  className="bg-emerald-50 text-emerald-600 hover:bg-emerald-100 border border-emerald-200 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl font-bold flex items-center gap-1.5 shadow-sm disabled:opacity-50 text-sm transition-colors"
                >
                ...
                <button 
                  ...
                  className="bg-slate-100 text-slate-700 dark:text-slate-300 dark:bg-slate-700/50 hover:bg-slate-200 border border-slate-200 dark:border-slate-600 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl font-bold flex items-center gap-1.5 shadow-sm text-sm transition-colors"
                >
                ...
                <button 
                  ...
                  className="bg-green-600 hover:bg-green-700 text-white px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl font-bold flex items-center gap-1.5 shadow-lg shadow-indigo-600/20 text-sm transition-all"
                >
                  <IndianRupee size={16} />
...
{/* Metric Summary */}
            {(() => {
              ...
              return (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-600">
                    <p className="text-slate-500 dark:text-slate-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-1">Total Billed</p>
                    <h3 className="text-xl sm:text-2xl font-black text-slate-800 dark:text-slate-100">₹{parseFloat(totalBilled).toFixed(2)}</h3>
                    <p className="text-slate-400 text-[10px] mt-1 font-semibold uppercase">Lifetime</p>
                  </div>
                  <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-600">
                    <p className="text-slate-500 dark:text-slate-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-1">Total Paid</p>
                    <h3 className="text-xl sm:text-2xl font-black text-indigo-600">₹{parseFloat(totalPaid).toFixed(2)}</h3>
                    <p className="text-slate-400 text-[10px] mt-1 font-semibold uppercase">Lifetime</p>
                  </div>
                  <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-600">
                    <p className="text-slate-500 dark:text-slate-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-1">Outstanding</p>
                    {advance > 0 ? (
                      <>
                        <h3 className="text-xl sm:text-2xl font-black text-emerald-500">₹0.00</h3>
                        <p className="text-emerald-500 text-[10px] mt-1 font-black uppercase">Advance: ₹{parseFloat(advance).toFixed(2)}</p>
                      </>
                    ) : (
                      <>
                        <h3 className={`text-xl sm:text-2xl font-black ${outstanding > 0 ? 'text-red-500' : 'text-emerald-500'}`}>₹{parseFloat(outstanding).toFixed(2)}</h3>
                  ...
                  <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-600">
                    <p className="text-slate-500 dark:text-slate-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-1">Total Bills</p>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-600 flex flex-col w-full sm:flex-1 sm:min-h-0">
               <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 flex flex-shrink-0 items-center gap-2">
               ...
               <div className="flex-1 overflow-y-auto w-full custom-scrollbar">
                 {ledgerEntries.length === 0 ? (
                    <div className="p-12 text-center text-slate-400 font-medium">No ledger transactions yet.</div>
                 ) : (
                    <>
                      {/* Desktop Render */}
                        ...
                        <table className="w-full text-left border-collapse min-w-full">
                          <thead className="bg-slate-50 dark:bg-slate-800/80 text-slate-500 dark:text-slate-400 text-xs uppercase font-bold tracking-wider sticky top-0 border-b border-slate-100 dark:border-slate-700 z-10">
                            <tr>
                              ...
                              <th 
className="py-4 px-6 text-right text-emerald-600 bg-emerald-50/30">Paid (Credit)</th>
                          ...
                          <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                            {ledgerEntries.reduce((acc, txn) => {
                              ...
                              const debit = (type === 'SALE' || type === 'OPENING') ? parseFloat(txn.amount || 0) : 0;
                              const credit = (type === 'PAYMENT' || type === 'PAYMENT_IN' || type === 'ROLLOVER') ? parseFloat(txn.amount || 0) : 0;
                              acc.balance = parseFloat((acc.balance + debit - credit).toFixed(2));
                              
                              acc.rows.push(
                                <tr key={`${type}-${txn.id || Date.now()}`} className="hover:bg-slate-50/80 dark:hover:bg-slate-700/50 transition-colors">
                                   <td className="py-4 px-6 text-slate-500 dark:text-slate-400 font-medium text-sm whitespace-nowrap">
                                   ...
                                   <td className="py-4 px-6 text-slate-800 dark:text-slate-100 font-bold text-sm">
                                      {type === 'SALE' && <span className="text-slate-500 border border-slate-200 bg-white px-2 py-0.5 rounded text-[10px] font-black uppercase mr-2">INV</span>}
                                      {(type === 'PAYMENT' || type === 'PAYMENT_IN') && <span className="text-emerald-600 border border-emerald-200 bg-emerald-50 px-2 py-0.5 rounded text-[10px] font-black uppercase mr-2">PAY</span>}
                                      {type === 'OPENING' && <span className="text-amber-500 border border-amber-200 bg-amber-50 px-2 py-0.5 rounded text-[10px] font-black uppercase mr-2">OPEN</span>}
                                      {txn.desc || txn.description || type}
                                   </td>
                                   <td className="py-4 px-6 text-right text-sm font-black text-slate-800 bg-red-50/10 dark:text-slate-100">
                                      {debit > 0 ? `₹${debit.toFixed(2)}` : '-'}
</td>
                                   <td className="py-4 px-6 text-right text-sm font-black text-emerald-600 bg-emerald-50/10">
                                      {credit > 0 ? `₹${credit.toFixed(2)}` : '-'}
                                   </td>
                                   <td className="py-4 px-6 text-right text-sm font-black text-indigo-600 bg-indigo-50/10">
                      ...
                      <div className="block sm:hidden p-4 space-y-3">
                        {ledgerEntries.reduce((acc, txn) => {
                          ...
                          if (type === 'SALE') { badgeColor = "bg-indigo-50 text-indigo-600 border-indigo-100"; amountColor = "text-red-500"; prefix = "Billed: "; }
                          if (type === 'PAYMENT' || type === 'PAYMENT_IN') { badgeColor = "bg-emerald-50 text-emerald-600 border-emerald-100"; amountColor = "text-emerald-600"; prefix = "Paid: "; }
                          if (type === 'OPENING') { badgeColor = "bg-amber-50 text-amber-600 border-amber-100"; amountColor = "text-amber-600"; }

                          acc.rows.push(
                            <div key={`${type}-${txn.id || Date.now()}`} className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-600 rounded-xl p-4 shadow-sm active:bg-slate-50 transition-colors">
                              <div className="flex justify-between items-start mb-2">
                        <div className="flex flex-col gap-1">
                                  <div className="flex items-center gap-2">
                                    <span className={`px-2 py-0.5 rounded-md text-[10px] border font-black uppercase tracking-wider ${badgeColor}`}>
                                      {type}
                                    </span>
                                  ...
                                  <span className="text-[10px] text-slate-400 font-bold tracking-widest">{new Date(txn.date || Date.now()).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                                </div>
            ...
            <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
               <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 focus:ring-2 focus:ring-indigo-500/50 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 font-medium outline-none"
                  />
               </div>
               <div>
...
className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 focus:ring-2 focus:ring-indigo-500/50 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 font-medium outline-none"
               ...
               <div>
                  <label className="text-gray-700 dark:text-gray-300 block text-sm font-bold mb-1.5">Add to Opening Balance</label>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full pl-11 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 focus:ring-2 focus:ring-indigo-500/50 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 font-bold outline-none"
                      placeholder="0.00"
                    />
                  </div>
               </div>
            ...
            <form onSubmit={handlePaymentSubmit} className="p-6 space-y-5">
              <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-600 p-4 rounded-xl text-center">
                ...
                <div className="mt-2 text-sm bg-white border border-slate-100 inline-block px-3 py-1 rounded-lg">
                   Due: <span className="font-black text-red-500">₹{parseFloat(getCustomerTotals(currentCustomer).outstanding || 0).toFixed(2)}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <IndianRupee size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-600" />
                  <input 
                    type="number" required min="0.01" step="0.01"
                    value={paymentForm.amount}
                    onChange={(e) => setPaymentForm({...paymentForm, amount: e.target.value})}
className="w-full pl-11 pr-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-emerald-500/50 bg-white text-emerald-600 text-xl font-black shadow-inner"
                    placeholder="0.00"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-emerald-500/50 bg-white text-slate-700 font-medium"
                />
              </div>
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-emerald-500/50 bg-slate-50 text-slate-700 font-medium"
                  placeholder="e.g. Cash payment"
                />
              </div>

              <div className="pt-2">
                <button type="submit" disabled={isSubmitting} className="w-full py-4 font-bold rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-400 text-white shadow-emerald-600/30">
