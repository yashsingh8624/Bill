...
export const calculateCustomerBalance = (ledger, customerId, customer = null) => {
  ...
  return Math.max(0, balance) || 0; // Never return negative for simple UI
};
...
// ========== SUPPLIER BALANCE ==========
// Formula: outstanding = SUM(PURCHASE) - SUM(PAYMENT_MADE + PAYMENT_OUT) + openingBalance
...
export const calculateSupplierBalance = (ledger, supplierId, supplier = null) => {
  ...
  let balance = ledger
    ...
    .reduce((sum, e) => {
      ...
      if (type === 'purchase') return sum + amt;
