...
import { getCollectionData, addDocument, updateDocument, queryDocuments } from '../utils/firestoreService';
...
export const BillProvider = ({ children }) => {
  ...
  const { spreadsheetId, folderId, isReady, useFirebase } = useAuth();
    ...
    try {
      ...
      if (useFirebase) {
        const [billsData, ledgerData] = await Promise.all([
          ...
          getCollectionData('ledger/all_transactions') // Reading from user specified exact path
        ]);
  ...
  }, [spreadsheetId, mapBillToApp, showToast, useFirebase]);
  ...
  const addBill = async (bill) => {
    try {
      ...
      if (useFirebase) {
        // === FIRESTORE ONLY — all data goes to ledger collection ===
        await addDocument('bills', newBill);
        for (const itemRow of itemRows) {
          await addDocument('bill_items', itemRow);
        }
        await addDocument('ledger', saleLedger);
        ...
        if (parseFloat(newBill.amount_paid) > 0) {
          ...
          await addDocument('ledger', payLedger);
        }
