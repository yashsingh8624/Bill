...
export default function NewBill() {
  ...
  // Prev balance UI completely removed as requested, now governed by toggle
  const [enablePrevBalance, setEnablePrevBalance] = useState(false);
  const customerPrevBalance = enablePrevBalance && selectedCustomerId ? calculateCustomerBalance(ledger, selectedCustomerId, customers.find(c => c.id === selectedCustomerId)) : 0;
  ...
  const handleSaveBill = async (isNew = false) => {
    ...
    const calculatedPrevBalance = (enablePrevBalance && custId)
        ? calculateCustomerBalance(ledger, custId, customers.find(c => c.id === custId))
        : 0;

    console.log('[DEBUG] --- PDF/Save Generation ---');
    console.log('Toggle State:', enablePrevBalance);
    ...
    if (isNew) {
      ...
      // Store current bill for sharing in overlay
  ...
  const handleGeneratePDF = () => {
    ...
    const calculatedPrevBalance = (enablePrevBalance && resolveCustId)
        ? calculateCustomerBalance(ledger, resolveCustId, customers.find(c => c.id === resolveCustId))
        : 0;

    console.log('[DEBUG] --- Manual PDF Generation ---');
    console.log('Toggle State (ON?):', enablePrevBalance);
          ...
          <div className="bg-white p-6 rounded-[20px] shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100/50 relative">
            <div className="flex justify-between items-center mb-4">
               ...
               <div className="flex items-center gap-3">
                 <span className="text-sm font-semibold text-slate-600">Previous Balance</span>
                 <button
                   type="button"
                   onClick={() => setEnablePrevBalance(!enablePrevBalance)}
                   className={`w-12 h-6 rounded-full transition-colors relative shadow-inner ${enablePrevBalance ? 'bg-indigo-600' : 'bg-slate-300'}`}
                 >
                   <div className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all shadow ${enablePrevBalance ? 'left-[26px]' : 'left-0.5'}`}></div>
                 </button>
               </div>
            </div>
