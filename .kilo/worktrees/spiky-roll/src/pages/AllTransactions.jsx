  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        if (useFirebase) {
          // Read from Firestore ledger collection — this is where all transactions live
          console.log('[AllTransactions] Reading from Firestore ledger...');
          ...
          data = ledgerData
            ...
            .map(entry => ({
              ...
              party_id: entry.customer_id || entry.supplier_id || '',
              type: mapLedgerType(entry.type),
              amount: entry.amount
            }));
          console.log(`[AllTransactions] Got ${data.length} transactions from ledger`);
        } else {
          // Sheets fallback
          if (!spreadsheetId) return;
          await ensureAllTransactionsSheet(spreadsheetId);
          data = await getSheetData(spreadsheetId, 'ALL_TRANSACTIONS');
        }
    ...
    fetchTransactions();
  }, [isReady, spreadsheetId, useFirebase, refreshTrigger]);
