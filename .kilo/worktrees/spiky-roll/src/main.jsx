...
// Wrapper that conditionally provides GoogleOAuth
function GoogleOAuthWrapper({ children }) {
  ...
  // Without a client ID, skip the provider (Firebase-only mode)
  return <>{children}</>;
}

if (!USE_FIREBASE && (!GOOGLE_CLIENT_ID || GOOGLE_CLIENT_ID === 'YOUR_CLIENT_ID_HERE')) {
