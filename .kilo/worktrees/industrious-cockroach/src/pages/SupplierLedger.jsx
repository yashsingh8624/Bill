        ...
        currentSupplier && (
          <div className="flex flex-col space-y-6 animate-in slide-in-from-right-4 duration-300 w-full pb-24">
            ...
            {(() => {
              ...
              const totalBilled = supplierTxns.filter(t => t && (t.type === 'PURCHASE' || t.type === 'SUPPLIER_OPENING')).reduce((s, e) => s + parseFloat(e.amount || 0), 0);
              ...
              const calculatedOutstanding = totalBilled - totalGiven;
              ...
              const totalBills = supplierTxns.filter(t => t && t.type === 'PURCHASE').length;
              return (
                        ...
                        <table className="w-full text-left border-collapse min-w-[650px]">
                          <thead className="bg-slate-50 text-slate-500 text-xs uppercase font-bold tracking-wider sticky top-0 border-b border-slate-100 z-10">
                            <tr>
                              ...
                              <th className="py-4 px-6 text-right text-rose-600 bg-rose-50/30">Payment Out</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-100">
                            {supplierTxns.map((txn, idx) => {
                                ...
                                <tr key={`${txn.id}-${idx}`} className="hover:bg-slate-50/80 transition-colors">
                                   ...
                                   <td className="py-4 px-6 text-slate-800 font-bold text-sm">
                                     {isPayment ? (
                                       <span className="text-rose-600 mr-2 border border-rose-200 bg-rose-50 px-2 py-0.5 rounded text-[10px] font-black tracking-wider uppercase">PAID</span>
                                     ) : (
                                     ...
                                     {txn.desc || (isPayment ? 'Payment Made' : 'Purchase')}
                                   </td>
                                   <td className={`py-4 px-6 text-right text-sm font-black ${isPayment ? 'text-rose-600 bg-rose-50/10' : 'text-slate-800'}`}>₹{parseFloat(txn.amount || 0).toFixed(2)}</td>
                                </tr>
                              )})}
                          </tbody>
                        </table>
