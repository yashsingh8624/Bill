                         ...
                         <div className="max-h-52 overflow-y-auto custom-scrollbar border border-slate-200 rounded-xl bg-white shadow-lg absolute w-full max-w-sm z-50">
                            {filteredCustomers.length === 0 ? (
                               ...
                               filteredCustomers.map(c => (
                                  <div key={c.id} onClick={() => setFormData({...formData, customerId: c.id, customerSearch: ''})} className="p-4 border-b border-slate-50 hover:bg-emerald-50 cursor-pointer flex items-center justify-between group">
                                     <span className="font-bold text-slate-700 group-hover:text-emerald-700">{c.name}</span>
                                     <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded uppercase group-hover:bg-emerald-200 group-hover:text-emerald-700">Select</span>
                                  </div>
                               ))
                            )}
                         </div>
