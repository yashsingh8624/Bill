...
/**
 * Execute a Firestore transaction for atomic operations
...
export const runFirestoreTransaction = async (transactionFn) => {
  try {
    ...
    console.error('[Firestore] ❌ Transaction FAILED:', err.code, err.message);
    throw err;
  }
};

/**
 * Atomic operation: Add bill with ledger entries and stock updates
 */
export const addBillTransaction = async (billData, ledgerEntries, stockUpdates) => {
  return runFirestoreTransaction(async (transaction) => {
    const uid = getUid();

    // Add bill
    const billRef = doc(collection(db, 'users', uid, 'bills'));
    transaction.set(billRef, { ...billData, _updatedAt: new Date().toISOString() });

    // Add all ledger entries to the unified ledger collection
    for (const entry of ledgerEntries) {
      const ledgerRef = doc(collection(db, 'users', uid, 'ledger', 'all_transactions', entry.id));
      transaction.set(ledgerRef, { ...entry, _updatedAt: new Date().toISOString() });
    }

    // Update stock
    for (const update of stockUpdates) {
      const productRef = doc(db, 'users', uid, 'products', update.productId);
      transaction.update(productRef, {
        quantity: update.newQuantity,
        stock_history_json: JSON.stringify(update.newHistory),
...
/**
 * Atomic operation: Add payment with ledger entry
 */
export const addPaymentTransaction = async (paymentData, ledgerEntry) => {
  return runFirestoreTransaction(async (transaction) => {
    const uid = getUid();

    // Add payment to unified ledger
    const ledgerRef = doc(collection(db, 'users', uid, 'ledger', 'all_transactions', ledgerEntry.id));
    transaction.set(ledgerRef, { ...ledgerEntry, _updatedAt: new Date().toISOString() });
...
/**
 * Atomic operation: Add invoice with ledger entry
 */
export const addInvoiceTransaction = async (invoiceData, ledgerEntry) => {
  return runFirestoreTransaction(async (transaction) => {
    const uid = getUid();

    // Add invoice to unified ledger
    const ledgerRef = doc(collection(db, 'users', uid, 'ledger', 'all_transactions', ledgerEntry.id));
    transaction.set(ledgerRef, { ...ledgerEntry, _updatedAt: new Date().toISOString() });
...
/**
 * Atomic operation: Update stock
 */
export const updateStockTransaction = async (productId, quantityChange, historyEntry) => {
  return runFirestoreTransaction(async (transaction) => {
    const uid = getUid();
    ...
    const currentQuantity = parseInt(currentData.quantity || 0, 10);
    const newQuantity = Math.max(0, currentQuantity + quantityChange);

    const currentHistory = currentData.stock_history_json ? JSON.parse(currentData.stock_history_json) : [];
    const newHistory = [...currentHistory, historyEntry];

    transaction.update(productRef, {
      quantity: String(newQuantity),
      stock_history_json: JSON.stringify(newHistory),
      _updatedAt: new Date().toISOString()
    });

    return { newQuantity, newHistory };
  });
};
