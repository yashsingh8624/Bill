...
files.forEach(file => {
    ...
    // Only apply if not already heavily modified to avoid duplicates in previously edited files
    ...
    content = content.replace(/text-slate-700(?!\s+dark:text-slate-200)/g, 'text-slate-700 dark:text-slate-200 transition-colors duration-300');
    
    // text-slate-600 -> text-slate-600 dark:text-slate-300
    content = content.replace(/text-slate-600(?!\s+dark:text-slate-300)/g, 'text-slate-600 dark:text-slate-300 transition-colors duration-300');
    
    // text-slate-500 -> text-slate-500 dark:text-slate-400
    content = content.replace(/text-slate-500(?!\s+dark:text-slate-400)/g, 'text-slate-500 dark:text-slate-400 transition-colors duration-300');
    ...
    // Usually "bg-indigo-600 hover:bg-indigo-700" -> "bg-green-600 hover:bg-green-700"
    // And "bg-indigo-600 hover:bg-indigo-500" -> "bg-green-600 hover:bg-green-500"
    // but limit it to primary actions if possible. We'll simply replace indigo with green where we want generic green buttons.
    // Actually, "change the primary buttons to green-600"
...
console.log("Dark mode injected across JSX files.");
